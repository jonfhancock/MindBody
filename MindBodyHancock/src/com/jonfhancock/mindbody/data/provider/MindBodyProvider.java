...
import com.jonfhancock.mindbody.data.provider.MindBodyContent.Appointment;
import com.jonfhancock.mindbody.data.provider.MindBodyContent.Staff;
 ...
 */
public final class MindBodyProvider extends ContentProvider {

    private static final String LOG_TAG = MindBodyProvider.class.getSimpleName();

    /* package */ static final boolean ACTIVATE_ALL_LOGS = true;

    protected static final String DATABASE_NAME = "MindBodyProvider.db";

    public static final String AUTHORITY = "com.jonfhancock.mindbody.provider.MindBodyProvider";
    ...
    private enum UriType {
        STAFF(Staff.TABLE_NAME, Staff.TABLE_NAME, Staff.TYPE_ELEM_TYPE),
        STAFF_ID(Staff.TABLE_NAME + "/#", Staff.TABLE_NAME, Staff.TYPE_DIR_TYPE),
        APPOINTMENT(Appointment.TABLE_NAME, Appointment.TABLE_NAME, Appointment.TYPE_ELEM_TYPE),
        APPOINTMENT_ID(Appointment.TABLE_NAME + "/#", Appointment.TABLE_NAME, Appointment.TYPE_DIR_TYPE);
    ...
    private class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            Log.d(LOG_TAG, "Creating MindBodyProvider database");

            // Create all tables here; each class has its own method
            Staff.createTable(db);
            Appointment.createTable(db);
        }
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            Staff.upgradeTable(db, oldVersion, newVersion);
            Appointment.upgradeTable(db, oldVersion, newVersion);
        }
    ...
    public int bulkInsert(Uri uri, ContentValues[] values) {
        ...
        try {
            switch (uriType) {
                case STAFF:
                    insertStmt = db.compileStatement(Staff.getBulkInsertString());
                    for (ContentValues value : values) {
                        Staff.bindValuesInBulkInsert(insertStmt, value);
                        insertStmt.execute();
                ...
                case APPOINTMENT:
                    insertStmt = db.compileStatement(Appointment.getBulkInsertString());
                    for (ContentValues value : values) {
                        Appointment.bindValuesInBulkInsert(insertStmt, value);
                        insertStmt.execute();
                        insertStmt.clearBindings();
                    }
                    insertStmt.close();
